...
| MLPolicy_5 | lr=0.00015, gamma=0.98, batch=8, epsilon_start=1.0, epsilon_end=0.02, epsilon_decay=0.35 | Observations (Mean Reward: 227.00 ± 46.05, Random Baseline: 142.50, Improvement: 84.50) |
 ...
 Hyperparameter Tuning Results: Nhial MajokPolicyHyperparameter SetNoted BehaviorCNNPolicylr=0.0001, $\gamma$=0.990, batch=8, $\epsilon_{\text{start}}=1.0, \epsilon_{\text{end}}=0.05, \epsilon_{\text{decay}}=0.30$Observations (Mean Reward: 275.00 ± 182.28, Random Baseline: 101.50, Improvement: 173.50). Analysis: Strong improvement but high variance ($\pm 182.28$), suggesting unstable learning.CNNPolicylr=0.0005, $\gamma$=0.950, batch=16, $\epsilon_{\text{start}}=1.0, \epsilon_{\text{end}}=0.10, \epsilon_{\text{decay}}=0.20$Observations (Mean Reward: 407.00 ± 247.00, Random Baseline: 143.75, Improvement: 263.25). Analysis: Achieved the highest mean reward but with the highest variance ($\pm 247.00$) due to the high learning rate ($\text{lr}=0.0005$).CNNPolicylr=0.0002, $\gamma$=0.990, batch=8, $\epsilon_{\text{start}}=1.0, \epsilon_{\text{end}}=0.01, \epsilon_{\text{decay}}=0.40$Observations (Mean Reward: 259.50 ± 85.19, Random Baseline: 130.75, Improvement: 128.75). Analysis: Moderate stability and performance. High $\gamma$ (0.99) promotes good long-term planning.CNNPolicylr=0.0003, $\gamma$=0.999, batch=12, $\epsilon_{\text{start}}=1.0, \epsilon_{\text{end}}=0.08, \epsilon_{\text{decay}}=0.25$Observations (Mean Reward: 285.00 ± 0.00, Random Baseline: 124.00, Improvement: 161.00). Analysis: Excellent stability ($\pm 0.00$). The near-perfect discount factor ($\gamma=0.999$) is critical for a robust policy.CNNPolicylr=0.00015, $\gamma$=0.980, batch=8, $\epsilon_{\text{start}}=1.0, \epsilon_{\text{end}}=0.02, \epsilon_{\text{decay}}=0.35$Observations (Mean Reward: 227.00 ± 46.05, Random Baseline: 142.50, Improvement: 84.50). Analysis: Solid improvement, limited by the conservative learning rate and lower $\gamma$.MLPPolicylr=0.0001, $\gamma$=0.990, batch=8, $\epsilon_{\text{start}}=1.0, \epsilon_{\text{end}}=0.05, \epsilon_{\text{decay}}=0.30$Observations (Mean Reward: 50.00 ± 0.00, Random Baseline: 181.00, Improvement: -131.00). Analysis: Complete failure. The MLP model cannot process pixel data, leading to performance significantly worse than a random agent.MLPPolicylr=0.0005, $\gamma$=0.950, batch=16, $\epsilon_{\text{start}}=1.0, \epsilon_{\text{end}}=0.10, \epsilon_{\text{decay}}=0.20$Observations (Mean Reward: 236.00 ± 33.23, Random Baseline: 148.75, Improvement: 87.25). Analysis: Surprisingly successful for an MLP. The larger batch size (16) provides stability despite the high learning rate ($\text{lr}=0.0005$).MLPPolicylr=0.0002, $\gamma$=0.990, batch=8, $\epsilon_{\text{start}}=1.0, \epsilon_{\text{end}}=0.01, \epsilon_{\text{decay}}=0.40$Observations (Mean Reward: 285.00 ± 0.00, Random Baseline: 174.00, Improvement: 111.00). Analysis: Perfect stability ($\pm 0.00$). The high $\gamma$ (0.99) is the dominant stabilizing factor.MLPPolicylr=0.0003, $\gamma$=0.999, batch=12, $\epsilon_{\text{start}}=1.0, \epsilon_{\text{end}}=0.08, \epsilon_{\text{decay}}=0.25$Observations (Mean Reward: 285.00 ± 0.00, Random Baseline: 124.00, Improvement: 161.00). Analysis: Highest MLP improvement and perfect stability ($\pm 0.00$).MLPPolicylr=0.00015, $\gamma$=0.980, batch=8, $\epsilon_{\text{start}}=1.0, \epsilon_{\text{end}}=0.02, $\epsilon_{\text{decay}}=0.35$Observations (Mean Reward: 227.00 ± 46.05, Random Baseline: 142.50, Improvement: 84.50). Analysis: Matches the CNN result, with moderate variance introduced by the small batch size.
